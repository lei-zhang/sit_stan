...
transformed parameters {
  ...
  for (s in 1:nSubjects) {
    ...
    tau[s]  <- Phi_approx( tau_mu_pr + beta[3,s] + tau_sd * tau_raw[s] ) * 10;
  }
}
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      chswtch[s,t] ~ bernoulli_logit( tau[s] * (valfun2[3-choice1[s,t]] - valfun2[choice1[s,t]]) );
...
generated quantities {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      log_likc2[s] <- log_likc2[s] + bernoulli_logit_log(chswtch[s,t], tau[s]*(valfun2_gen[3-choice1[s,t]] - valfun2_gen[choice1[s,t]]) );

      c_rep[s,t]   <- bernoulli_rng( inv_logit( tau[s]*(valfun2_gen[3-choice1[s,t]] - valfun2_gen[choice1[s,t]]) ) );
