data {
  ...
  real<lower=0,upper=100> stayBYswitch[nSubjects];
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      if ( chswtch[s,t] == 1 ) {
        increment_log_prob( bernoulli_logit_log( chswtch[s,t], valfun2) - stayBYswitch[s] );
      } else {
        increment_log_prob( bernoulli_logit_log( chswtch[s,t], valfun2) );
      }
...
generated quantities {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      if ( chswtch[s,t] == 1 ) {
        log_likc2[s] <- log_likc2[s] + bernoulli_logit_log(chswtch[s,t], valfun2_gen) - stayBYswitch[s];
      } else {
