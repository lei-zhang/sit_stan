data {
  ...
  matrix<lower=-1,upper=1>[nTrials,4]  otherReward2[nSubjects]; // dim: [ns, t, 4]
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      valfun2 <- beta[3,s] + (beta[4,s]*(valdiff>=0) + beta[5,s]*(valdiff<0)) * valdiff + 
                (beta[6,s]*(wgtWith[s,t]>=wgtAgst[s,t]) + beta[7,s]*(wgtWith[s,t]<wgtAgst[s,t])) * (wgtWith[s,t] - wgtAgst[s,t]);
      ...
      if (t==1) {
        otherReward_vec  <- otherCumAcc[s,t] .* otherReward2[s,t];
      } else if (t==2) {
        otherReward_vec  <- otherCumAcc[s,t] .* otherReward2[s,t] + otherCumAcc[s,t-1] .* otherReward2[s,t-1] * disc[s];
      } else {
        disc_mat <- rep_matrix(exp(log(disc[s])*pwr),4); // replicate by 4 columns 
        otherReward_vec  <- rep_row_vector(1.0, 3) * (disc_mat .* block(otherCumAcc[s], t-2, 1, 3, 4) .* block(otherReward2[s], t-2, 1, 3, 4));
      }
...
generated quantities {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      valfun2_gen <- beta[3,s] + (beta[4,s]*(valdiff_gen>=0) + beta[5,s]*(valdiff_gen<0)) * valdiff_gen + 
                (beta[6,s]*(wgtWith[s,t]>=wgtAgst[s,t]) + beta[7,s]*(wgtWith[s,t]<wgtAgst[s,t])) * (wgtWith[s,t] - wgtAgst[s,t]);
      ...
      if (t==1) {
        otherReward_vec2  <- otherCumAcc[s,t] .* otherReward2[s,t];
      } else if (t==2) {
        otherReward_vec2  <- otherCumAcc[s,t] .* otherReward2[s,t] + otherCumAcc[s,t-1] .* otherReward2[s,t-1] * disc[s];
      } else {
        disc_mat2 <- rep_matrix(exp(log(disc[s])*pwr),4); // replicate by 4 columns 
        otherReward_vec2  <- rep_row_vector(1.0, 3) * (disc_mat2 .* block(otherCumAcc[s], t-2, 1, 3, 4) .* block(otherReward2[s], t-2, 1, 3, 4));
      }
