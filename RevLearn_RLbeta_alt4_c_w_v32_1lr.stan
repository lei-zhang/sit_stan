...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      valfun2[choice1[s,t]]   <- beta[3,s] * valfun1[choice1[s,t]] + beta[4,s] * wgtWith[s,t];
      valfun2[3-choice1[s,t]] <- beta[3,s] * valfun1[3-choice1[s,t]] + beta[5,s] * wgtAgst[s,t];

      choice2[s,t] ~ categorical_logit( valfun2 );
...
generated quantities {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      valfun2_gen[choice1[s,t]]   <- beta[3,s] * valfun1_gen[choice1[s,t]] + beta[4,s] * wgtWith[s,t];
      valfun2_gen[3-choice1[s,t]] <- beta[3,s] * valfun1_gen[3-choice1[s,t]] + beta[5,s] * wgtAgst[s,t];

      log_likc2[s] <- log_likc2[s] + categorical_logit_log(choice2[s,t], valfun2_gen);
      c_rep[s,t]   <- categorical_rng( softmax(valfun2_gen) );
