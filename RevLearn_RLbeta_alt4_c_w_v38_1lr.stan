...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      valdiff <- bet1[s,t] * (valfun1[choice1[s,t]] - valfun1[3-choice1[s,t]]);
      valfun2 <- beta[3,s] + beta[4,s] * valdiff + 
                (beta[5,s]*(wgtWith[s,t]>=wgtAgst[s,t]) + beta[6,s]*(wgtWith[s,t]<wgtAgst[s,t])) * (wgtWith[s,t] - wgtAgst[s,t]);
...
generated quantities {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      valdiff_gen  <- bet1[s,t] * (valfun1_gen[choice1[s,t]] - valfun1_gen[3-choice1[s,t]]);
      valfun2_gen  <- beta[3,s] + beta[4,s] * valdiff_gen + 
                     (beta[5,s]*(wgtWith[s,t]>=wgtAgst[s,t]) + beta[6,s]*(wgtWith[s,t]<wgtAgst[s,t])) * (wgtWith[s,t] - wgtAgst[s,t]);
