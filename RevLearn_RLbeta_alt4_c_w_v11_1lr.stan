...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      if (t==1) {
        otherReward_norm <- (wOthers[s,t] .* otherReward2[s,t]) / ( sum( wOthers[s,t] .* otherReward2[s,t]) + machine_precision() );
      } else if (t==2) {
        otherReward_norm <- ( wOthers[s,t] .* otherReward2[s,t] + wOthers[s,t-1] .* otherReward2[s,t-1] * disc[s] ) / 
                               ( sum( wOthers[s,t] .* otherReward2[s,t] + wOthers[s,t-1] .* otherReward2[s,t-1]*disc[s] ) + machine_precision() );
      } else {
        disc_mat <- rep_matrix(exp(log(disc[s])*pwr),4); // replicate by 4 columns 
        otherReward_norm <- ( rep_row_vector(1.0, 3) * (disc_mat .* block(wOthers[s], t-2, 1, 3, 4) .* block(otherReward2[s], t-2, 1, 3, 4)) ) / 
                               ( sum( disc_mat .* block(wOthers[s], t-2, 1, 3, 4) .* block(otherReward2[s], t-2, 1, 3, 4) ) + machine_precision() );
      }

      otherValue[t+1,choice2[s,t]]   <- sum( othW     .* otherReward_norm );
      otherValue[t+1,3-choice2[s,t]] <- sum( (1-othW) .* otherReward_norm );
    }  // trial loop
  }    // subject loop
}
...
generated quantities {
  ...
  row_vector[4] othW2;
  row_vector[4] otherReward_norm2;
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      if (t==1) {
        otherReward_norm2 <- (wOthers[s,t] .* otherReward2[s,t]) / ( sum( wOthers[s,t] .* otherReward2[s,t]) + machine_precision() );
      } else if (t==2) {
        otherReward_norm2 <- ( wOthers[s,t] .* otherReward2[s,t] + wOthers[s,t-1] .* otherReward2[s,t-1] * disc[s] ) / 
                                ( sum( wOthers[s,t] .* otherReward2[s,t] + wOthers[s,t-1] .* otherReward2[s,t-1]*disc[s] ) + machine_precision() );
      } else {
        disc_mat2 <- rep_matrix(exp(log(disc[s])*pwr),4); // replicate by 4 columns 
        otherReward_norm2 <- ( rep_row_vector(1.0,3) * (disc_mat2 .* block(wOthers[s], t-2, 1, 3, 4) .* block(otherReward2[s], t-2, 1, 3, 4)) ) / 
                                ( sum( disc_mat2 .* block(wOthers[s], t-2, 1, 3, 4) .* block(otherReward2[s], t-2, 1, 3, 4) ) + machine_precision() );
      }

      otherValue2[t+1,choice2[s,t]]   <- sum( othW2     .* otherReward_norm2 );
      otherValue2[t+1,3-choice2[s,t]] <- sum( (1-othW2) .* otherReward_norm2 );
    }  // trial loop
  }    // subject loop
}