...
model {
  ...
  vector[2] v2[nTrials+1];    // 2nd values
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      choice2[s,t] ~ categorical_logit(tau[s] * v2[t]);
      ...
      if (choice1[s,t]==choice2[s,t]) {
          v1[t+1] <- v2[t] + lr[s] * pe[t];
      } else {
          v1[t+1][choice2[s,t]]   <- v2[t][choice2[s,t]]   + lr[s] * pe[t];
          v1[t+1][3-choice2[s,t]] <- v2[t][3-choice2[s,t]] + lr[s] * penc[t];  
      } // if
    }
  }
}
...
generated quantities {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      log_lik2[s] <- log_lik2[s] + categorical_logit_log(choice2[s,t], tau[s] * ev2[t]);
      
      pe2[t]   <-  reward[s,t] - ev2[t][choice2[s,t]];
      penc2[t] <- -reward[s,t] - ev2[t][3-choice2[s,t]];

      if (choice1[s,t]==choice2[s,t]) {
          ev1[t+1] <- ev2[t] + lr[s] * pe2[t];
      } else {
          ev1[t+1][choice2[s,t]]   <- ev2[t][choice2[s,t]]   + lr[s] * pe2[t];
          ev1[t+1][3-choice2[s,t]] <- ev2[t][3-choice2[s,t]] + lr[s] * penc2[t];  
      }
    }
  }
}